#!/usr/bin/env python3
"""
FireGun CLI – Firebase pentest & vuln scanner (v1.2)
...
USER_AGENT = "FireGun/1.2 (+https://example.local)"
...
async def http_request_with_backoff(
    ...
    """Performs an HTTP request with exponential backoff (+ jitter) for rate limiting and server errors."""
    delay = initial_delay
    last_exc: Optional[Exception] = None
    for attempt in range(max_retries):
        try:
            resp: httpx.Response = await request_func()
            if resp.status_code in (429,) or resp.status_code >= 500:
                if attempt < max_retries - 1:
                    jitter = random.uniform(0, delay / 3)
                    sleep_for = delay + jitter
                    print(f"[!] HTTP {resp.status_code}. Retrying in {sleep_for:.2f}s...", file=sys.stderr)
            ...
            if attempt < max_retries - 1:
                jitter = random.uniform(0, delay / 3)
                sleep_for = delay + jitter
                print(f"[-] Request error: {e}. Retrying in {sleep_for:.2f}s...", file=sys.stderr)
                await asyncio.sleep(sleep_for)
                delay *= 2
            else:
                raise
    # Fallback (should not reach)
    raise ConnectionError(f"Failed to complete request after {max_retries} retries: {last_exc}")
...
async def _identity_toolkit_request(url: str, api_key: str, email: str, password: str) -> Optional[str]:
    ...
    try:
        async with httpx.AsyncClient(headers={"User-Agent": USER_AGENT}, follow_redirects=True) as client:
            ...
            r.raise_for_status()
            data = r.json()
            print(f"[+] Success. UID={data.get('localId')}")
...
async def exploit_rtdb(client: httpx.AsyncClient, root_url: str):
    ...
    payload = {"__firegun_warning__": msg}
    # Keep bandwidth low
    request_func = lambda: client.patch(root_url + ("&" if "?" in root_url else "?") + "print=silent", json=payload, timeout=10)
    r = await http_request_with_backoff(request_func)
...
async def scan_rtdb(client: httpx.AsyncClient, raw: str, sem: asyncio.Semaphore,
    ...
    try:
        async with sem:
            # Read check (shallow, tolerant of empty DB returning null)
            ...
            if resp.status_code == 200:
                # null indicates readable but empty; JSON error indicates blocked
                try:
                    data = resp.json()
                    if isinstance(data, (dict, list)) or data is None:
                        can_read = True
                except json.JSONDecodeError:
                    can_read = "Permission denied" not in resp.text

            # Write probe (silent)
            try:
                write_check_func = lambda: client.put(probe_url + "&print=silent" if "?" in probe_url else probe_url + "?print=silent",
                                                     json={"probe": "firegun"})
        ...
        status = "/".join(parts) if parts else f"SECURE (HTTP {resp.status_code})"
        pretty = f"{raw} [{'VULNERABLE' if parts else 'SECURE'} : {status}]"
        print(pretty)

        if results is not None:
            results.append({"target": raw, "type": "rtdb", "read": can_read, "write": can_write})
...
def _fs_headers(id_token: Optional[str]) -> Dict[str, str]:
    h = {"User-Agent": USER_AGENT}
    if id_token:
        # Note: Firestore v1 prefers OAuth2 access tokens; however, many rulesets allow unauthenticated access.
        # We still send Firebase ID token when provided; servers may ignore it.
        h["Authorization"] = f"Bearer {id_token}"
    return h

async def _fs_can_read(client: httpx.AsyncClient, base_url: str, api_key: str, headers: Dict[str, str]) -> Tuple[bool, Optional[int]]:
    # Probe a non-existent doc to distinguish 404 (readable path) vs 403/401 (blocked)
    url = f"{base_url}/__firegun__/__probe_read__?key={api_key}"
    r = await http_request_with_backoff(lambda: client.get(url, headers=headers))
    if r.status_code in (200, 404):
        return True, r.status_code
    return False, r.status_code

async def _fs_try_create_doc(client: httpx.AsyncClient, base_url: str, api_key: str, headers: Dict[str, str]) -> Tuple[bool, Optional[int]]:
    # Create in a top-level collection; use createDocument semantics
    url = f"{base_url}/__firegun__?documentId=__probe_write__&key={api_key}"
    payload = {"fields": {"probe": {"stringValue": "firegun"}}}
    r = await http_request_with_backoff(lambda: client.post(url, headers=headers, json=payload))
    if r.status_code in (200, 201):
        # cleanup
        del_url = f"{base_url}/__firegun__/__probe_write__?key={api_key}"
        try:
...
async def enumerate_firestore_collections(client: httpx.AsyncClient, base_url: str, api_key: str, headers: Dict[str, str]):
    ...
    try:
        ...
        resp = await http_request_with_backoff(lambda: client.post(list_collections_url, headers=headers, json={"pageSize": 50}))
        if resp.status_code == 200:
            ...
            for c in collections[:5]:
                ...
                docs_resp = await http_request_with_backoff(lambda: client.get(docs_url, headers=headers))
                if docs_resp.status_code == 200:
                    docs = docs_resp.json().get("documents", [])
                    doc_names = [d['name'].rsplit('/', 1)[-1] for d in docs]
                    print(f"      - {c} sample docs: {doc_names}")
        elif resp.status_code in (401, 403):
            print("  [-] Collection enumeration blocked by rules (401/403)")
...
async def scan_firestore(client: httpx.AsyncClient, project: str, api_key: str,
    ...
    headers = _fs_headers(id_token)
    can_read, can_write = False, False
    last_code = None

    try:
        async with sem:
            can_read, last_code = await _fs_can_read(client, base_url, api_key, headers)
            if exploit:
                can_write, last_code = await _fs_try_create_doc(client, base_url, api_key, headers)

        parts = []
        if can_read: parts.append("READ")
        if can_write: parts.append("WRITE")
        status = "/".join(parts) if parts else f"SECURE (HTTP {last_code})"
        print(f"Firestore {project} [{'VULNERABLE' if parts else 'SECURE'} : {status}]")

        if results is not None:
            results.append({"project": project, "type": "firestore", "read": can_read, "write": can_write})
...
def _parse_storage_url(url: str) -> Tuple[Optional[str], Optional[str]]:
    """Extract bucket and object path from a Firebase Storage download URL.
    Returns (bucket, object_path) or (None, None) if parsing fails.
    """
    try:
        p = urlparse(url)
        parts = [x for x in p.path.strip('/').split('/') if x]
        # Expect formats like: v0/b/<bucket>/o/<encoded object>
        if len(parts) >= 5 and parts[0] in {"v0", "v0-b"} and parts[1] == "b" and parts[3] == "o":
            bucket = parts[2]
            obj_enc = parts[4]
            obj = unquote(obj_enc)
            return bucket, obj
...
async def _storage_try_upload(client: httpx.AsyncClient, bucket: str, id_token: Optional[str]) -> Tuple[bool, str]:
    headers = {"User-Agent": USER_AGENT, "Content-Type": "text/plain"}
    if id_token:
        headers["Authorization"] = f"Bearer {id_token}"
    data = b"firegun_probe"

    # Try Firebase v0 endpoint first
    url_v0 = f"https://firebasestorage.googleapis.com/v0/b/{bucket}/o?name=firegun_probe.txt&uploadType=media"
    r = await http_request_with_backoff(lambda: client.post(url_v0, headers=headers, content=data))
    if r.status_code in (200, 201):
        return True, "v0"

    # Fallback to Cloud Storage JSON upload endpoint (requires OAuth token; may fail with Firebase ID token)
    url_gcs = f"https://storage.googleapis.com/upload/storage/v1/b/{bucket}/o?uploadType=media&name=firegun_probe.txt"
    r2 = await http_request_with_backoff(lambda: client.post(url_gcs, headers=headers, content=data))
    if r2.status_code in (200, 201):
        return True, "gcs"

    return False, ""

async def _storage_try_delete(client: httpx.AsyncClient, bucket: str, id_token: Optional[str]) -> None:
    headers = {"User-Agent": USER_AGENT}
    if id_token:
        headers["Authorization"] = f"Bearer {id_token}"
    # Try both APIs; ignore errors
    url_v0 = f"https://firebasestorage.googleapis.com/v0/b/{bucket}/o/{quote('firegun_probe.txt', safe='')}"
    try:
        await client.delete(url_v0, headers=headers)
    except Exception:
        pass
    url_gcs = f"https://storage.googleapis.com/storage/v1/b/{bucket}/o/{quote('firegun_probe.txt', safe='')}"
    try:
        await client.delete(url_gcs, headers=headers)
    except Exception:
        pass

async def scan_storage(client: httpx.AsyncClient, url: str, sem: asyncio.Semaphore, exploit: bool,
                       id_token: Optional[str] = None, results: Optional[List[Dict[str, Any]]] = None):
    can_read, can_write = False, False
    resp: Optional[httpx.Response] = None
    bucket, _ = _parse_storage_url(url)

    try:
        async with sem:
            # 1) READ check (attempt direct GET alt=media)
            read_url = url.split("?")[0] + "?alt=media"
            resp = await http_request_with_backoff(lambda: client.get(read_url))
            if resp.status_code == 200:
                can_read = True

            # 2) WRITE check (optional)
            if exploit and bucket:
                try:
                    ok, which = await _storage_try_upload(client, bucket, id_token)
                    if ok:
                        can_write = True
                finally:
                    if can_write:
                        await _storage_try_delete(client, bucket, id_token)
    ...
    if can_read: parts.append("READ")
    if can_write: parts.append("WRITE")
    status = "/".join(parts) if parts else f"SECURE (HTTP {resp.status_code if resp else 'N/A'})"
    print(f"{url} [{'VULNERABLE' if parts else 'SECURE'} : {status}]")

    if results is not None:
        results.append({"url": url, "type": "storage", "read": can_read, "write": can_write})
...
async def run_scan(rtdb_targets: List[str], storage_targets: List[str], concurrency: int, exploit: bool,
                   auth_token: Optional[str], readout: bool, json_mode: bool):
    headers = {"User-Agent": USER_AGENT}
    async with httpx.AsyncClient(timeout=30, headers=headers, follow_redirects=True) as client:
        ...
        if rtdb_targets:
            tasks.extend([scan_rtdb(client, t, sem, exploit, auth_token, readout, results) for t in rtdb_targets])
        if storage_targets:
            tasks.extend([scan_storage(client, u, sem, exploit, auth_token, results) for u in storage_targets])
        if tasks:
...
async def run_fs_scan(projects: List[str], api_key: str, id_token: Optional[str], concurrency: int, exploit: bool, json_mode: bool):
    headers = {"User-Agent": USER_AGENT}
    async with httpx.AsyncClient(timeout=30, headers=headers, follow_redirects=True) as client:
        sem = asyncio.Semaphore(concurrency)
        results: List[Dict[str, Any]] = []
        tasks = [scan_firestore(client, pj, api_key, id_token, sem, exploit, results) for pj in projects]
        await asyncio.gather(*tasks)
        if json_mode:
            print(json.dumps(results, indent=2))

async def run_scan_storage(urls: List[str], concurrency: int, exploit: bool, id_token: Optional[str], json_mode: bool):
    headers = {"User-Agent": USER_AGENT}
    async with httpx.AsyncClient(timeout=20, headers=headers, follow_redirects=True) as client:
        sem = asyncio.Semaphore(concurrency)
        results: List[Dict[str, Any]] = []
        await asyncio.gather(*(scan_storage(client, u, sem, exploit, id_token, results) for u in urls))
        if json_mode:
            print(json.dumps(results, indent=2))
...
def handle_scan(args: argparse.Namespace) -> None:
    ...
    asyncio.run(run_scan(rtdb_targets, storage_targets, args.concurrency, args.exploit, auth_token, args.readout, args.json))
...
def handle_fs_scan(args: argparse.Namespace) -> None:
    ...
    asyncio.run(run_fs_scan(projects, api_key, id_token, args.concurrency, args.exploit, args.json))
...
def handle_storage_scan(args: argparse.Namespace) -> None:
    ...
    asyncio.run(run_scan_storage(urls, args.concurrency, args.exploit, id_token, args.json))
...
def handle_fuzz_rules(args: argparse.Namespace) -> None:
    ...
    cmd = ["node", str(args.script), "--project", args.project, "--apiKey", api_key, "--rules", str(args.rules)]
    if id_token:
        cmd += ["--token", id_token]
    run_node_script(cmd)
...
def main():
    ...
    scan_parser.add_argument("-e", "--exploit", action="store_true", help="Attempt to write a non-destructive warning to vulnerable targets")
    scan_parser.add_argument("-r", "--readout", action="store_true", help="Print readable data from vulnerable RTDBs")
    scan_parser.add_argument("--config", type=Path, help="JSON config file with idToken or auth token")
    scan_parser.add_argument("--json", action="store_true", help="Emit JSON results to stdout")
    ...
    fs_scan_parser.add_argument("--json", action="store_true", help="Emit JSON results to stdout")
    ...
    st_parser.add_argument("--exploit", action="store_true", help="Try a dummy upload to probe WRITE access (to firegun_probe.txt)")
    st_parser.add_argument("--id-token", help="Optional Firebase ID token for authenticated checks (or FIREBASE_ID_TOKEN)")
    st_parser.add_argument("--config", type=Path, help="JSON config file with idToken or auth token")
    st_parser.add_argument("--json", action="store_true", help="Emit JSON results to stdout")
